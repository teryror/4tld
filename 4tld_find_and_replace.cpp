                           ...
                           User_Input input)
{
    if (input.key.keycode == key_back) {
        ...
        char  * append_pos = bar->string.str         + bar->string.size;
        int32_t append_len = bar->string.memory_size - bar->string.size;
        bar->string.size += clipboard_index(app, 0, 0, append_pos, append_len);
        
        if (bar->string.size > bar->string.memory_size) {
            // NOTE: This happens sometimes because clipboard_index returns the
            // length of the clipboard entry, not the number of bytes written.
            bar->string.size = bar->string.memory_size;
        }
    } else if (key_is_unmodified(&input.key)) {
        uint8_t character[4];
        uint32_t length = to_writable_character(input, character);
        if (length != 0 && bar->string.memory_size - bar->string.size > (int32_t)length) {
            append_ss(&bar->string, make_string((char *) &character, length));
        }
    }
}
